...
implementation 
	...
	function TMessagingThread.QueryInterface({$IFDEF FPC_HAS_CONSTREF}constref{$ELSE}const{$ENDIF} IID: TGUID; out Obj):HResult; {$IFNDEF WINDOWS}cdecl{$ELSE}stdcall{$ENDIF};
	begin if GetInterface(IID, Obj) then Result := 0 else Result := HResult($80004002); end;
	function TMessagingThread._AddRef: Integer; {$IFNDEF WINDOWS}cdecl{$ELSE}stdcall{$ENDIF}; begin Result := 1; end;
	function TMessagingThread._Release: Integer; {$IFNDEF WINDOWS}cdecl{$ELSE}stdcall{$ENDIF}; begin Result := 1; end;
